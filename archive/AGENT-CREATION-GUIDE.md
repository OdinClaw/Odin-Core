...
Primary model: [e.g., minimax/MiniMax-M2.5]
Fallbacks: [list per escalation chain — default: Claude Haiku → Sonnet → Groq → Ollama]
...
- **Model:** MiniMax M2.5 primary (same escalation chain as Odin)
...
## Pantheon Framework Files (Added 2026-03-08)

As of the Pantheon kernel upgrade, every agent workspace requires **3 additional files** beyond the 6-file identity stack. These are mandatory for all new agents and are being backfilled for existing agents.

### New Required Files

#### manifest.yaml
The agent's source-of-truth configuration card. Machine-readable.
```
workspace/agents/<id>/manifest.yaml
```
Contains: id, name, type (isolated|subagent), status, model config, Discord routing, workspace paths, role, dependencies (reads/writes/notifies), tags, activation checklist.

Template: `~/.openclaw-odin/kernel/templates/manifest.yaml`

#### mission.md
The agent's operational brief for the current phase. Human-readable.
```
workspace/agents/<id>/mission.md
```
Contains: standing mission (permanent function), current phase + focus, active tasks table, blockers, success metric, phase history.

Different from SOUL.md (who you ARE) and IDENTITY.md (your place in the system). MISSION = what you're doing RIGHT NOW.

Template: `~/.openclaw-odin/kernel/templates/mission.md`

#### skills.md
The agent's capabilities inventory. Updated when skills change.
```
workspace/agents/<id>/skills.md
```
Contains: active skills table, skills in progress, skills needed, known limitations, tools + integrations.

Template: `~/.openclaw-odin/kernel/templates/skills.md`

---

### Updated Full Checklist (9 files per agent)

```
IDENTITY STACK (existing):
[ ] SOUL.md
[ ] IDENTITY.md
[ ] USER.md
[ ] AGENTS.md
...
NEW FRAMEWORK FILES (required as of 2026-03-08):
[ ] manifest.yaml
[ ] mission.md
[ ] skills.md

ACTIVATION STEPS (unchanged):
[ ] openclaw agents add <id> --profile odin
[ ] mkdir workspace/agents/<id>/memory
[ ] Copy auth-profiles.json to agents/<id>/agent/
[ ] Add agents.agents.<id> block to openclaw.json
[ ] Add channel routing to openclaw.json channels
[ ] Restart gateway
[ ] Test in Discord channel
```
...
The Pantheon kernel directory contains governance documents that inform agent behavior:

| File | Purpose |
|------|---------|
| `kernel/DOCTRINE.md` | The laws of the Pantheon — read to understand the system's governing principles |
| `kernel/PANTHEON-REGISTRY.md` | Master agent registry — update when agent status changes |
| `kernel/MEMORY-PROMOTION.md` | How memory graduates from session → shared context → kernel |
| `kernel/DISCORD-MIRROR.md` | Which files map to which channels and update frequency |
| `kernel/MIGRATION-PLAN.md` | Activation priority order for remaining agents |

Kernel location: `~/.openclaw-odin/kernel/`

---

### Handoff Schema

For inter-agent task passing, use the handoff template:
```
kernel/templates/handoff.md
```
Write completed handoffs to: `workspace/agents/<to-agent>/handoffs/YYYY-MM-DD-<task-slug>.md`

---

_Updated 2026-03-08: Added Pantheon framework file requirements._
